...
// Delete User: using pathVariable
server.delete("/auth/users/delete/:email", (req, res) => {
  ...
  if (exist_email !== -1) {
      ...
      const status = 200;
      return res.status(status).json({ status, message: "User deleted successfully" });
  } else {
      return res.status(401).json({
          status: 401,
...
//Create order
server.post("/auth/order/create", (req, res) => {
  const { id, customerName, bookId, quantity, timestamp } = req.body;

  exist_order = db.books.findIndex((x) => x.id === bookId);
  if (exist_order === -1) {
      return res.status(401).json({
          status: 401,
          message: "BookId not exist!",
      });
  }
